...
void Keeper::delBookFromXML(const std::string& compare)
{
    ...
    if (doc.SaveFile(book_last_name.c_str()) == XML_SUCCESS)
    {
        std::cout << "Book remove. Books  saved:" << book_last_name
                  << std::endl;
    }
}

void Keeper::saveGivenBookToXML(
    const std::multimap<Reader_iter, Book_iter>& givenBook)
{
    XMLDocument doc  = new XMLDocument;
...
std::multimap<std::string, std::string> Keeper::readGivenBookfromXML()
{
    ...
    if (!doc.Error())
    {
        ...
        for (auto link = root->FirstChildElement(); link;
